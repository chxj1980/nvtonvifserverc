...
void test_Get() {
	...
	Node node = list1->get(list1, 1);
	TEST_ASSERT_NOT_EQUAL(NULL, node);
	TEST_ASSERT_EQUAL(10, *((int*)(node->element)));
}

void for_each_p(Node node) {
...
void test_ForEach() {
	...
	list1->forEach(list1, for_each_p);
}
